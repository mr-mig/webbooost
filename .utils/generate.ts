...
const fullPathConfigPath = j(rootPath, 'src', 'configs', 'fullpath-gen.ts')
const partsConfigPath = j(rootPath, 'src', 'configs', 'parts-gen.ts')
...
function generateFullPathConfig(injectees: Injectee[]) {
        ...
        injectee.descriptor.urls.forEach(
          cdnUrl => {
            cdnUrl = cdnUrl
              ...
              .replace('$filepath$', injectee.descriptor.filepath)
...
function generatePartsConfig(injectees: Injectee[]) {
  const mapping: { [k: string]: string } = {}

  injectees.reduce(
    (map, injectee) => { 
      injectee.files.forEach(file => {
        injectee.descriptor.parts?.forEach(
          regex => {
            regex = regex
              .replace('$version$', file.version)
              .replace('$filetype$', file.type)
...
const fullpath = generateFullPathConfig(injectees)
const parts = generatePartsConfig(injectees)

const result = 
  `📦 Total injectees: ${injectees.length}
📦 Total files: ${injectees.reduce((s, j) => s + j.files.length, 0)}

🔡 Fullpath config generated. Total rules: ${Object.keys(fullpath).length}
🔎 Parts config generated. Total rules:${Object.keys(parts).length}
`


console.log(result);

w(toTSFile(fullpath), fullPathConfigPath)
console.log(`📥 Hash config written to ${fullPathConfigPath}`)

w(toTSFile(parts), partsConfigPath)
console.log(`📥 Reg config written to ${partsConfigPath}`)
